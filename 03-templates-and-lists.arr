...
fun remove-n(l :: List<Number>, n :: Number) -> List<Number>:
  cases (List<Number>) l:
    | empty => empty
    | link(f, r) =>
      if not(f == n):
...
where:
  remove-n([list: 1, 2, 3], 3) is [list: 1, 2]
  remove-n([list: 1, 2], 3) is [list: 1, 2]
  remove-n(empty, 3) is empty
  remove-n([list: 3, 3, 3], 3) is empty
end
